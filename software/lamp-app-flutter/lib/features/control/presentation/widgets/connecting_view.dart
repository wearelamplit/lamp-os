...
/// Full-screen "connecting to the lamp" state: a critter SVG that gently
/// pulses, with a single line of text below. Derives the critter from the
/// lamp's `lampId` so it matches what every user sees for this lamp;
/// falls back to `deviceId` for legacy entries with no mirrored `lampId`.
...
class _ConnectingViewState extends ConsumerState<ConnectingView>
  ...
  Widget build(BuildContext context) {
    ...
    final asset = critterAssetFor(
      (lamp?.lampId?.isNotEmpty ?? false) ? lamp!.lampId! : widget.deviceId,
    );
    // Switches the message during the post-save reconnect window.
