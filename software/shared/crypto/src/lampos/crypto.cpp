...
bool deriveKey(const uint8_t* salt, size_t saltLen,
  ...
  // info = "lamp-v1" 0x00 charShortName. The NUL is a separator byte inside
  // the info buffer, not a C-string terminator, so build the buffer
  // explicitly instead of relying on strlen.
...
bool decryptPayload(const uint8_t* p, size_t n,
