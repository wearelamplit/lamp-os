...
//      32, kDevPubkey)` — i.e. the message we verify against is
...
//
// The signature offset (32) is fixed by this layout; the 4 reserved bytes at
// [28..32) are the only slack a field can grow into without a version bump.
//
// The signature covers `SHA256(image[0 .. signedRegionLen))`. The
// public key against which we verify is embedded as
// kDevPubkey[32] in components/catch_ota/dev_pubkey.h (generated
...
//     verifies against kDevPubkey over the SHA-256 digest of the
...
//
// In production, the OTA receive state machine constructs a reader that
// wraps esp_partition_read against the inactive OTA partition. In native
// tests, the reader is backed by a std::vector<uint8_t> fixture.
