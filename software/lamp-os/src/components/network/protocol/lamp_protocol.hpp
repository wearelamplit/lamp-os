...
//                   MSG_WISP_PALETTE (0x26), MSG_WISP_PAINT (0x27) — wisp coordination.
//   override.hpp    MSG_OVERRIDE_COLORS (0x21) / RESTORE_COLORS (0x22) /
//                   OVERRIDE_BRIGHTNESS (0x23) / RESTORE_BRIGHTNESS (0x24).
//   event.hpp       MSG_EVENT (0x30)                     — expression-fired announce.
//   command.hpp     MSG_COMMAND (0x31)                   — targeted expression invocation.
...
// constant); control_op + presence before MAX_PACKET_SIZE below (it
// aggregates their max sizes). Each header also includes its own deps,
...
// MAX_PACKET_SIZE: receiver buffer sizing. CONTROL_MAX_SIZE is 250 (the
// ESP-NOW cap); other families are smaller. Use a max() over the candidates
// so a future shrink of any one doesn't silently shrink the buffer.
