...
// lamp_internal.hpp is private to lamp.cpp + lamp_drains.cpp, so the receiver
// is forward-declared extern here in the same shape.
...
#ifdef LAMP_DEBUG
  // Per-tick flicker diagnostic: log which branch fired and the live state
  // probes. Catches isInProgress()/totalChunks() flapping mid-session (the
  // symptom is the strip alternating between progress bar and dim-only
  // fill). Logs once per indicator paint call; the static-state dedupe
  // prints only branch transitions to keep the volume sane.
...
#ifdef LAMP_DEBUG
  // Rate-limited paint diagnostic: log what the indicator computes about
  // every 500 ms to correlate visible flicker with internal state. Logs
  // once across both frame buffers; the tracking variable is function-
  // static, scoped to the FIRST FB call each render cycle.
