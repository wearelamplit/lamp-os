...
namespace lamp {
// Typed-payload posts called from ShowReceiver's WiFi recv task. Same
...
void lamp::Lamp::setup() {
  ...
  // work. MUST be installed BEFORE showReceiver.begin(), or any CONTROL_OP
  ...
  // Install the OTA receiver BEFORE showReceiver.begin() so the chunk
  ...
  // populated after the EspNowLink::begin() inside showReceiver.begin(), so
  // receiver-side init has to run AFTER showReceiver.begin(). The
  ...
  // ACCEPT/REQ/RESULT via show_receiver's dispatch ladder).
