// Native-host unit tests for firmware_signature::verifySignedFirmware.
//
// We link the production firmware_signature.cpp; its native path uses
...
// Native test rig convention (matches test_tuple_sampler — the production
...
// verify call. mbedTLS streaming SHA-256 is taken straight from the
// host's mbedtls install (PIO's native env links against system mbedtls
// via the build flags below — see platformio.ini's native env).
...
//
// firmware_signature.cpp's native path calls this extern. We track each
...
// Production firmware_signature.cpp declares this as `extern int` inside
// namespace lamp::firmware (line referenced from verifySignedFirmware's
// native path). The definition has to live in the same namespace so the
// linker resolves to this mock and not a libsodium symbol.
...
// Bring production firmware_signature.cpp into this TU (after the mock is
...
static std::vector<uint8_t> buildImage(
  ...
  std::vector<uint8_t> img(signedLen + lf::kLsigFooterLen, 0);
  for (size_t i = 0; i < signedLen; ++i) img[i] = static_cast<uint8_t>(i ^ 0xA5);
  uint8_t* footer = img.data() + signedLen;
  std::memcpy(footer + lf::kLsigMagicOffset, magic, 4);
  std::memset(footer + lf::kLsigChannelOffset, 0, lf::kLsigChannelLen);
  size_t chLen = std::strlen(channel);
  if (chLen > lf::kLsigChannelLen) chLen = lf::kLsigChannelLen;
  std::memcpy(footer + lf::kLsigChannelOffset, channel, chLen);
  // version (LE)
  footer[lf::kLsigVersionOffset + 0] = static_cast<uint8_t>(version & 0xFF);
  footer[lf::kLsigVersionOffset + 1] = static_cast<uint8_t>((version >> 8) & 0xFF);
  footer[lf::kLsigVersionOffset + 2] = static_cast<uint8_t>((version >> 16) & 0xFF);
  footer[lf::kLsigVersionOffset + 3] = static_cast<uint8_t>((version >> 24) & 0xFF);
  // signedRegionLen (LE)
  footer[lf::kLsigSignedLenOffset + 0] = static_cast<uint8_t>(embeddedSignedLen & 0xFF);
  footer[lf::kLsigSignedLenOffset + 1] = static_cast<uint8_t>((embeddedSignedLen >> 8) & 0xFF);
  footer[lf::kLsigSignedLenOffset + 2] = static_cast<uint8_t>((embeddedSignedLen >> 16) & 0xFF);
  footer[lf::kLsigSignedLenOffset + 3] = static_cast<uint8_t>((embeddedSignedLen >> 24) & 0xFF);
  // signature: arbitrary bytes (mocked verifier doesn't actually check).
  for (size_t i = 0; i < lf::kLsigSignatureLen; ++i) {
    footer[lf::kLsigSignatureOffset + i] = static_cast<uint8_t>(0xBE ^ i);
  }
  return img;
}
...
static lf::FirmwareByteReader makeVectorReader(const std::vector<uint8_t>& v) {
...
void test_verify_happy_path() {
  ...
  const bool ok = lf::verifySignedFirmware(makeVectorReader(img), img.size(),
...
void test_verify_bad_magic_rejected() {
  ...
  footer[lf::kLsigMagicOffset + 0] = 'X';  // was 'L'
...
void test_verify_too_short_image_rejected() {
  resetMock();
  std::vector<uint8_t> tiny(lf::kLsigFooterLen - 1, 0);
  const bool ok = lf::verifySignedFirmware(makeVectorReader(tiny), tiny.size(),
                                           nullptr, nullptr);
  TEST_ASSERT_FALSE(ok);
}
...
void test_verify_null_reader_rejected() {
  ...
  const bool ok = lf::verifySignedFirmware(lf::FirmwareByteReader{}, 1000,
                                           nullptr, nullptr);
  TEST_ASSERT_FALSE(ok);
}
...
void test_verify_full_8_byte_channel() {
  resetMock();
  // Channel takes the full 8-byte slot with no terminator on the wire.
  auto img = buildImage(/*signedLen=*/256, /*embedded=*/256,
                        "LSIG", 0x42, "abcdefgh");
  const char* outChannel = nullptr;
  uint32_t outVersion = 0;
  const bool ok = lf::verifySignedFirmware(makeVectorReader(img), img.size(),
                                           &outChannel, &outVersion);
  TEST_ASSERT_TRUE(ok);
  // The returned buffer is null-terminated at index 8 — fits in the
  // 9-byte module-static slot.
...
void test_verify_out_params_optional() {
  ...
  TEST_ASSERT_TRUE(lf::verifySignedFirmware(makeVectorReader(img), img.size(),
                                             nullptr, nullptr));
}
...
void test_verify_reader_short_read_rejected() {
  ...
  const bool ok = lf::verifySignedFirmware(shortReader, img.size(),
                                           nullptr, nullptr);
  TEST_ASSERT_FALSE(ok);
}
