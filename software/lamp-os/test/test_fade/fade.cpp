// Native-host unit tests for the fade/ease functions in src/util/fade.cpp.
//
// Context: audit finding [HIGH] flagged the two 512-entry LUTs
// (quadEaseInOut, linear) as living in heap+RAM via std::vector. The fix
// migrates them to `static constexpr uint32_t[512]` so they live in
// .rodata (flash). The refactor is mechanical — same data, same indexing
// — so this test pins the observable behavior of ease()/easeLinear()/
// fade()/fadeLinear() so the refactor is provably behavior-preserving.
//
// Following test_gradient/test_color/test_invocation convention: the
// functions under test are re-implemented inline here. The native test
// env doesn't link src/. The LUTs reproduced below MUST match
// src/util/fade.cpp byte-for-byte; if you change the LUTs in src, mirror
// here (or delete this file — it's a regression net for the audit fix).
//
// The LUT data here is *intentionally* duplicated as `static constexpr`
// (the same representation the production fix uses) — that way the test
// also acts as a syntax check that constexpr-512-uint32_t initializers
// compile under the native env's C++ standard.
...
uint8_t easeLinear(uint8_t start, uint8_t end, uint32_t duration, uint32_t currentStep) {
  if (currentStep >= duration) return end;
  uint32_t idx = (uint16_t)((currentStep * 511 / duration * 511) / 511);
  uint32_t factor = idx * 511;
  return (((end - start) * factor) / 262144) + start;
}
...
// --- LUT spot-checks: pins the exact data table values. -------------------
// Any drift here means the refactor changed the data (it shouldn't).

void test_lut_quad_endpoints() {
  TEST_ASSERT_EQUAL_UINT32(0u, lamp::quadEaseInOut[0]);
  TEST_ASSERT_EQUAL_UINT32(261118u, lamp::quadEaseInOut[510]);
}

void test_lut_quad_midpoint() {
  // Quad ease-in-out at midpoint ≈ half of max.
  TEST_ASSERT_EQUAL_UINT32(131071u, lamp::quadEaseInOut[256]);
}

void test_lut_linear_endpoints() {
  TEST_ASSERT_EQUAL_UINT32(0u, lamp::linear[0]);
  TEST_ASSERT_EQUAL_UINT32(260610u, lamp::linear[510]);
}

void test_lut_linear_midpoint() {
  // linear[256] = 256 * 511 = 130816
  TEST_ASSERT_EQUAL_UINT32(130816u, lamp::linear[256]);
}

// easeLinear() dropped the table for the closed form idx*511. This pins that
// the closed form equals the old table across the whole index range, so a
// future rounding/overflow regression trips.
void test_linear_closed_form_matches_table() {
  for (uint32_t i = 0; i < 511; ++i) {
    TEST_ASSERT_EQUAL_UINT32(lamp::linear[i], i * 511u);
  }
}

// --- ease() contract: start at 0, end at duration. ------------------------

void test_ease_at_step_zero_returns_start() {
  // step 0 → idx 0 → LUT[0] = 0 → factor 0 → returns start.
  TEST_ASSERT_EQUAL_UINT8(0, lamp::ease(0, 255, 100, 0));
  TEST_ASSERT_EQUAL_UINT8(42, lamp::ease(42, 200, 100, 0));
}

void test_ease_at_duration_returns_end_exact() {
  // currentStep >= duration short-circuits to `end` — no LUT lookup.
  TEST_ASSERT_EQUAL_UINT8(255, lamp::ease(0, 255, 100, 100));
  TEST_ASSERT_EQUAL_UINT8(255, lamp::ease(0, 255, 100, 200));
  TEST_ASSERT_EQUAL_UINT8(13, lamp::ease(200, 13, 50, 50));
}

void test_ease_when_start_equals_end_returns_end() {
  TEST_ASSERT_EQUAL_UINT8(7, lamp::ease(7, 7, 100, 50));
}

// --- easeLinear() contract: start at 0, end at duration, midpoint sane. ---

void test_ease_linear_at_step_zero_returns_start() {
  TEST_ASSERT_EQUAL_UINT8(0, lamp::easeLinear(0, 255, 100, 0));
}

void test_ease_linear_at_duration_returns_end_exact() {
  TEST_ASSERT_EQUAL_UINT8(255, lamp::easeLinear(0, 255, 100, 100));
}

void test_ease_linear_midpoint_is_approximately_half() {
  // ease(0, 200, 100, 50): idx = 50*511/100 = 255 → LUT[255] = 130305.
  // result = (200 * 130305) / 262144 = ~99.43 → 99.
  uint8_t result = lamp::easeLinear(0, 200, 100, 50);
  TEST_ASSERT_EQUAL_UINT8(99, result);
}

// --- fade()/fadeLinear() wire channels through correctly. -----------------

void test_fade_endpoints_per_channel() {
  lamp::Color start(0, 50, 100, 150);
  lamp::Color end(200, 100, 50, 0);

  // step 0 → start values
  lamp::Color at0 = lamp::fade(start, end, 100, 0);
  TEST_ASSERT_EQUAL_UINT8(0, at0.r);
  TEST_ASSERT_EQUAL_UINT8(50, at0.g);
  TEST_ASSERT_EQUAL_UINT8(100, at0.b);
  TEST_ASSERT_EQUAL_UINT8(150, at0.w);

  // step >= duration → end values exactly
  lamp::Color atEnd = lamp::fade(start, end, 100, 100);
  TEST_ASSERT_EQUAL_UINT8(200, atEnd.r);
  TEST_ASSERT_EQUAL_UINT8(100, atEnd.g);
  TEST_ASSERT_EQUAL_UINT8(50, atEnd.b);
  TEST_ASSERT_EQUAL_UINT8(0, atEnd.w);
}

void test_fade_linear_endpoints_per_channel() {
  lamp::Color start(10, 20, 30, 40);
  lamp::Color end(250, 200, 150, 100);

  lamp::Color at0 = lamp::fadeLinear(start, end, 100, 0);
  TEST_ASSERT_EQUAL_UINT8(10, at0.r);
  TEST_ASSERT_EQUAL_UINT8(20, at0.g);
  TEST_ASSERT_EQUAL_UINT8(30, at0.b);
  TEST_ASSERT_EQUAL_UINT8(40, at0.w);

  lamp::Color atEnd = lamp::fadeLinear(start, end, 100, 100);
  TEST_ASSERT_EQUAL_UINT8(250, atEnd.r);
  TEST_ASSERT_EQUAL_UINT8(200, atEnd.g);
  TEST_ASSERT_EQUAL_UINT8(150, atEnd.b);
  TEST_ASSERT_EQUAL_UINT8(100, atEnd.w);
}
...
int main(int argc, char **argv) {
  ...
  RUN_TEST(test_lut_quad_endpoints);
  RUN_TEST(test_lut_quad_midpoint);
  RUN_TEST(test_lut_linear_endpoints);
  RUN_TEST(test_lut_linear_midpoint);
  RUN_TEST(test_linear_closed_form_matches_table);

  RUN_TEST(test_ease_at_step_zero_returns_start);
  RUN_TEST(test_ease_at_duration_returns_end_exact);
  RUN_TEST(test_ease_when_start_equals_end_returns_end);

  RUN_TEST(test_ease_linear_at_step_zero_returns_start);
  RUN_TEST(test_ease_linear_at_duration_returns_end_exact);
  RUN_TEST(test_ease_linear_midpoint_is_approximately_half);

  RUN_TEST(test_fade_endpoints_per_channel);
  RUN_TEST(test_fade_linear_endpoints_per_channel);
