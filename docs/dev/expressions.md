...
- `void control()`, overridable but not required. The base class implementation handles the auto-trigger cadence, `onUpdate` dispatch, and `onComplete` dispatch. Subclasses that override `control()` take on the responsibility of those behaviours themselves; the continuous ones (breathing, spotty, flicker) route through the protected `Expression::continuousControl()` helper for the auto-retrigger.
...
- `wispDimFloor()` is a pure type-property, `1.0` by default (ignores the wisp, always full). `breathing`, `shifty`, `spotty`, and `flicker` return `0.3` (dim-and-blend under the wisp). `glitchy` and `pulse` keep the `1.0` default — brief flashes / waves read fine over a held wisp colour.
